...
(* NOTE: This interface mirrors the OxCaml stdlib hashtbl.mli exactly
   (oxcaml commit 2fa029274, 2026-06-09), with module references renamed
   Hashtbl -> Swiss_neon. This is the NEON (arm64, native-only) 16-wide
   instantiation of the same engine as Swiss; see docs/design.md. *)
...
val create : ?random: (* thwart tools/sync_stdlib_docs *) bool ->
             int -> ('a, 'b) t
(** [Swiss_neon.create n] creates a new, empty hash table, with
   ...
   execution of [Swiss_neon.create] or deterministic over all executions.
...
(** [Swiss_neon.add tbl key data] adds a binding of [key] to [data]
   in table [tbl].
...
val find : ('a, 'b) t -> 'a -> 'b
(** [Swiss_neon.find tbl x] returns the current binding of [x] in [tbl],
   or raises [Not_found] if no such binding exists. *)

val find_opt : ('a, 'b) t -> 'a -> 'b option
(** [Swiss_neon.find_opt tbl x] returns the current binding of [x] in [tbl],
    or [None] if no such binding exists.
    @since 4.05 *)

val find_all : ('a, 'b) t -> 'a -> 'b list
(** [Swiss_neon.find_all tbl x] returns the list of all data
   associated with [x] in [tbl].
   The current binding is returned first, then the previous
   bindings, in reverse order of introduction in the table. *)

val mem : ('a, 'b) t -> 'a -> bool
(** [Swiss_neon.mem tbl x] checks if [x] is bound in [tbl]. *)

val remove : ('a, 'b) t -> 'a -> unit
(** [Swiss_neon.remove tbl x] removes the current binding of [x] in [tbl],
   restoring the previous binding if it exists.
   It does nothing if [x] is not bound in [tbl]. *)

val replace : ('a, 'b) t -> 'a -> 'b -> unit
(** [Swiss_neon.replace tbl key data] replaces the current binding of [key]
...
(** [Swiss_neon.iter f tbl] applies [f] to all bindings in table [tbl].
...
val filter_map_inplace: ('a -> 'b -> 'b option) -> ('a, 'b) t ->
    unit
(** [Swiss_neon.filter_map_inplace f tbl] applies [f] to all bindings in
...
(** [Swiss_neon.fold f tbl init] computes
...
(** [Swiss_neon.length tbl] returns the number of bindings in [tbl].
   It takes constant time.  Multiple bindings are counted once each, so
   [Swiss_neon.length] gives the number of times [Swiss_neon.iter] calls its
   first argument. *)

val randomize : unit -> unit
(** After a call to [Swiss_neon.randomize()], hash tables are created in
    ...
    in {!create} call [Swiss_neon.randomize()] at initialization
    time before any domains are created.

    Note that once [Swiss_neon.randomize()] was called, there is no way
    to revert to the non-randomized default behavior of {!create}.
    This is intentional.  Non-randomized hash tables can still be
    created using [Swiss_neon.create ~random:false].
...
(** Return a copy of the given hashtable.  Unlike {!copy},
    ...
    by an old version of the {!Swiss_neon} module, then marshaled to
    persistent storage.  After unmarshaling, apply {!rebuild}
    to produce a hash table for the current version of the {!Swiss_neon}
    module.
...
(** [Swiss_neon.stats tbl] returns statistics about the table [tbl]:
...
(** The functorial interface allows the use of specific comparison
    ...
    {[
      ...
      module IntHashtbl = Swiss_neon.Make(IntHash)
    ...
    the type [('a,'b) Swiss_neon.t] of the generic interface. For
    example, [Swiss_neon.length h] would not type-check, you must use
    [IntHashtbl.length].
*)
...
(** Functor building an implementation of the hashtable structure.
    The functor [Swiss_neon.Make] returns a structure containing
...
(** Like [Make], but takes a portable [hash] function to
    portable [Swiss_neon] operations. *)
...
(** Functor building an implementation of the hashtable structure.
    The functor [Swiss_neon.MakeSeeded] returns a structure containing
    a type [key] of keys and a type ['a t] of hash tables
    associating data of type ['a] to keys of type [key].
    The operations perform similarly to those of the generic
    ...
    or if randomization is globally on (see {!Swiss_neon.randomize}).
    @since 4.00 *)
...
val hash : 'a -> int
(** [Swiss_neon.hash x] associates a nonnegative integer to any value of
...
(** [Swiss_neon.hash_param meaningful total x] computes a hash value for [x],
...
(** A variant of {!hash_param} that is further parameterized by
   ...
   [Swiss_neon.seeded_hash_param meaningful total seed x].
   @since 4.00 *)
...
(** {1:examples Examples}
  ...
  {[
    ...
    # let tbl =
        ...
        |> Swiss_neon.of_seq
    val tbl : (int, string) Swiss_neon.t = <abstr>

    # Swiss_neon.length tbl
    - : int = 100

    # Swiss_neon.find_opt tbl 32
    - : string option = Some "32"

    # Swiss_neon.find_opt tbl 166
    - : string option = None

    # Swiss_neon.replace tbl 166 "one six six"
    - : unit = ()
  ...
  {[
    # module Char_tbl = Swiss_neon.Make(struct
        type t = char
        let equal = Char.equal
        let hash = Swiss_neon.hash
      end)
