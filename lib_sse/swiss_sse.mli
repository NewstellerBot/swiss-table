...
(* NOTE: This interface mirrors the OxCaml stdlib hashtbl.mli exactly
   ...
   Hashtbl -> Swiss_sse. This is the SSE2 (amd64, native-only) 16-wide
...
val create : ?random: (* thwart tools/sync_stdlib_docs *) bool ->
             int -> ('a, 'b) t
(** [Swiss_sse.create n] creates a new, empty hash table, with
   ...
   execution of [Swiss_sse.create] or deterministic over all executions.
...
(** [Swiss_sse.add tbl key data] adds a binding of [key] to [data]
   in table [tbl].
...
val find : ('a, 'b) t -> 'a -> 'b
(** [Swiss_sse.find tbl x] returns the current binding of [x] in [tbl],
   or raises [Not_found] if no such binding exists. *)

val find_opt : ('a, 'b) t -> 'a -> 'b option
(** [Swiss_sse.find_opt tbl x] returns the current binding of [x] in [tbl],
    or [None] if no such binding exists.
    @since 4.05 *)

val find_all : ('a, 'b) t -> 'a -> 'b list
(** [Swiss_sse.find_all tbl x] returns the list of all data
   associated with [x] in [tbl].
   The current binding is returned first, then the previous
   bindings, in reverse order of introduction in the table. *)

val mem : ('a, 'b) t -> 'a -> bool
(** [Swiss_sse.mem tbl x] checks if [x] is bound in [tbl]. *)

val remove : ('a, 'b) t -> 'a -> unit
(** [Swiss_sse.remove tbl x] removes the current binding of [x] in [tbl],
   restoring the previous binding if it exists.
   It does nothing if [x] is not bound in [tbl]. *)

val replace : ('a, 'b) t -> 'a -> 'b -> unit
(** [Swiss_sse.replace tbl key data] replaces the current binding of [key]
...
(** [Swiss_sse.iter f tbl] applies [f] to all bindings in table [tbl].
...
val filter_map_inplace: ('a -> 'b -> 'b option) -> ('a, 'b) t ->
    unit
(** [Swiss_sse.filter_map_inplace f tbl] applies [f] to all bindings in
...
(** [Swiss_sse.fold f tbl init] computes
...
(** [Swiss_sse.length tbl] returns the number of bindings in [tbl].
   It takes constant time.  Multiple bindings are counted once each, so
   [Swiss_sse.length] gives the number of times [Swiss_sse.iter] calls its
   first argument. *)

val randomize : unit -> unit
(** After a call to [Swiss_sse.randomize()], hash tables are created in
    ...
    in {!create} call [Swiss_sse.randomize()] at initialization
    time before any domains are created.

    Note that once [Swiss_sse.randomize()] was called, there is no way
    to revert to the non-randomized default behavior of {!create}.
    This is intentional.  Non-randomized hash tables can still be
    created using [Swiss_sse.create ~random:false].
...
(** Return a copy of the given hashtable.  Unlike {!copy},
    ...
    by an old version of the {!Swiss_sse} module, then marshaled to
    persistent storage.  After unmarshaling, apply {!rebuild}
    to produce a hash table for the current version of the {!Swiss_sse}
    module.
...
(** [Swiss_sse.stats tbl] returns statistics about the table [tbl]:
...
(** The functorial interface allows the use of specific comparison
    ...
    {[
      ...
      module IntHashtbl = Swiss_sse.Make(IntHash)
    ...
    the type [('a,'b) Swiss_sse.t] of the generic interface. For
    example, [Swiss_sse.length h] would not type-check, you must use
    [IntHashtbl.length].
*)
...
(** Functor building an implementation of the hashtable structure.
    The functor [Swiss_sse.Make] returns a structure containing
...
(** Functor building an implementation of the hashtable structure.
    The functor [Swiss_sse.MakeSeeded] returns a structure containing
    a type [key] of keys and a type ['a t] of hash tables
    associating data of type ['a] to keys of type [key].
    The operations perform similarly to those of the generic
    ...
    or if randomization is globally on (see {!Swiss_sse.randomize}).
    @since 4.00 *)
...
val hash : 'a -> int
(** [Swiss_sse.hash x] associates a nonnegative integer to any value of
...
(** [Swiss_sse.hash_param meaningful total x] computes a hash value for [x],
...
(** A variant of {!hash_param} that is further parameterized by
   ...
   [Swiss_sse.seeded_hash_param meaningful total seed x].
   @since 4.00 *)
...
(** {1:examples Examples}
  ...
  {[
    ...
    # let tbl =
        ...
        |> Swiss_sse.of_seq
    val tbl : (int, string) Swiss_sse.t = <abstr>

    # Swiss_sse.length tbl
    - : int = 100

    # Swiss_sse.find_opt tbl 32
    - : string option = Some "32"

    # Swiss_sse.find_opt tbl 166
    - : string option = None

    # Swiss_sse.replace tbl 166 "one six six"
    - : unit = ()
  ...
  {[
    # module Char_tbl = Swiss_sse.Make(struct
        type t = char
        let equal = Char.equal
        let hash = Swiss_sse.hash
      end)
