...
(* NOTE: This interface mirrors the OxCaml stdlib hashtbl.mli exactly
   ...
   Hashtbl -> Swiss. See docs/design.md for documented behavioral deltas. *)
...
val create : ?random: (* thwart tools/sync_stdlib_docs *) bool ->
             int -> ('a, 'b) t
(** [Swiss.create n] creates a new, empty hash table, with
   ...
   execution of [Swiss.create] or deterministic over all executions.
...
(** [Swiss.add tbl key data] adds a binding of [key] to [data]
   in table [tbl].
...
val find : ('a, 'b) t -> 'a -> 'b
(** [Swiss.find tbl x] returns the current binding of [x] in [tbl],
   or raises [Not_found] if no such binding exists. *)

val find_opt : ('a, 'b) t -> 'a -> 'b option
(** [Swiss.find_opt tbl x] returns the current binding of [x] in [tbl],
    or [None] if no such binding exists.
    @since 4.05 *)

val find_all : ('a, 'b) t -> 'a -> 'b list
(** [Swiss.find_all tbl x] returns the list of all data
   associated with [x] in [tbl].
   The current binding is returned first, then the previous
   bindings, in reverse order of introduction in the table. *)

val mem : ('a, 'b) t -> 'a -> bool
(** [Swiss.mem tbl x] checks if [x] is bound in [tbl]. *)

val remove : ('a, 'b) t -> 'a -> unit
(** [Swiss.remove tbl x] removes the current binding of [x] in [tbl],
   restoring the previous binding if it exists.
   It does nothing if [x] is not bound in [tbl]. *)

val replace : ('a, 'b) t -> 'a -> 'b -> unit
(** [Swiss.replace tbl key data] replaces the current binding of [key]
...
(** [Swiss.iter f tbl] applies [f] to all bindings in table [tbl].
...
val filter_map_inplace: ('a -> 'b -> 'b option) -> ('a, 'b) t ->
    unit
(** [Swiss.filter_map_inplace f tbl] applies [f] to all bindings in
...
(** [Swiss.fold f tbl init] computes
...
(** [Swiss.length tbl] returns the number of bindings in [tbl].
   It takes constant time.  Multiple bindings are counted once each, so
   [Swiss.length] gives the number of times [Swiss.iter] calls its
   first argument. *)

val randomize : unit -> unit
(** After a call to [Swiss.randomize()], hash tables are created in
...
(** Return a copy of the given hashtable.  Unlike {!copy},
    ...
    by an old version of the {!Swiss} module, then marshaled to
    persistent storage.  After unmarshaling, apply {!rebuild}
    to produce a hash table for the current version of the {!Swiss}
    module.
...
(** [Swiss.stats tbl] returns statistics about the table [tbl]:
...
(** The functorial interface allows the use of specific comparison
    ...
    {[
      ...
      module IntHashtbl = Swiss.Make(IntHash)
    ...
    the type [('a,'b) Swiss.t] of the generic interface. For
    example, [Swiss.length h] would not type-check, you must use
    [IntHashtbl.length].
*)
...
(** Functor building an implementation of the hashtable structure.
    The functor [Swiss.Make] returns a structure containing
...
(** Functor building an implementation of the hashtable structure.
    The functor [Swiss.MakeSeeded] returns a structure containing
    a type [key] of keys and a type ['a t] of hash tables
    associating data of type ['a] to keys of type [key].
    The operations perform similarly to those of the generic
    ...
    or if randomization is globally on (see {!Swiss.randomize}).
    @since 4.00 *)
...
val hash : 'a -> int
(** [Swiss.hash x] associates a nonnegative integer to any value of
...
(** [Swiss.hash_param meaningful total x] computes a hash value for [x],
...
(** A variant of {!hash_param} that is further parameterized by
   ...
   [Swiss.seeded_hash_param meaningful total seed x].
   @since 4.00 *)
...
(** {1:examples Examples}
  ...
  {[
    ...
    # let tbl =
        ...
        |> Swiss.of_seq
    val tbl : (int, string) Swiss.t = <abstr>

    # Swiss.length tbl
    - : int = 100

    # Swiss.find_opt tbl 32
    - : string option = Some "32"

    # Swiss.find_opt tbl 166
    - : string option = None

    # Swiss.replace tbl 166 "one six six"
    - : unit = ()
  ...
  {[
    # module Char_tbl = Swiss.Make(struct
        type t = char
